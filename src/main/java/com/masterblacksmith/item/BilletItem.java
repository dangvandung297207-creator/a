...
/** A consolidated bar of metal. Its temperature bar is its life. */
public class BilletItem extends Item {
    private final String metalId;

    public BilletItem(String metalId) {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> lines, TooltipFlag flag) {
        ...
        lines.add(Component.translatable("tooltip.masterblacksmith.purity",
                String.format("%.1f%%", ForgingData.getPurity(stack))).withStyle(ChatFormatting.GRAY));
        String template = ForgingData.peek(stack).getString("Template");
        if (template.isEmpty()) {
            lines.add(Component.translatable("tooltip.masterblacksmith.needs_template").withStyle(ChatFormatting.DARK_GRAY));
        }
    }
}
